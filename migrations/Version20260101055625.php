 ...
 */
final class Version20260101055625 extends AbstractMigration
{
    /**
     * @inheritDoc
     */
    ...
    {
        return 'Update existing category values to match new enum';
    }// end getDescription()
    ...
    public function up(Schema $schema): void
    {
        $this->addSql(
            'CREATE TEMPORARY TABLE __temp__inventory_item AS
                SELECT id, location_id, name, description, inventory_number,
                    serial_number, category, specifications, created_at, updated_at FROM inventory_item'
        );
        ...
        $this->addSql(
            'INSERT INTO inventory_item (
                id, location_id, name, description, inventory_number,
                serial_number, category, specifications, created_at, updated_at
            )
            SELECT id, location_id, name, description, inventory_number,
                serial_number,
                CASE
                    WHEN category = \'pc\' THEN \'computer\'
                    WHEN category = \'furniture\' THEN \'other\'
                    WHEN category = \'monitor\' THEN \'monitor\'
                    WHEN category = \'printer\' THEN \'printer\'
                    WHEN category = \'other\' THEN \'other\'
                    ELSE \'other\'
                END as category,
                specifications,
                created_at,
                updated_at FROM __temp__inventory_item'
        );
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql(
            ...
            SELECT id, location_id, name, description, inventory_number, serial_number,
                CASE
                    WHEN category = \'computer\' THEN \'pc\'
                    WHEN category = \'furniture\' THEN \'other\'
                    WHEN category = \'monitor\' THEN \'monitor\'
                    WHEN category = \'printer\' THEN \'printer\'
                    WHEN category = \'other\' THEN \'other\'
                    ELSE \'other\'
                END as category, specifications, created_at, updated_at FROM __temp__inventory_item'
        );
        $this->addSql('DROP TABLE __temp__inventory_item');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_55BDEA30964C83FF ON inventory_item (inventory_number)');
        $this->addSql('CREATE INDEX IDX_55BDEA3064D218E ON inventory_item (location_id)');
